...
def showCategory(catalog_id):
    ...
    categoryItems = \
        session.query(CategoryItem).filter_by(category_id=catalog_id).all()

    # Get count of category items

    categoryItemsCount = \
        session.query(CategoryItem).filter_by(category_id=catalog_id).count()
...
def showCategoryItem(catalog_id, item_id):
    ...
    categoryItem = \
        session.query(CategoryItem).filter_by(id=item_id).first()
